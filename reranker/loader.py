  ...
  def __getitem__(self, idx):
      ...
      for item in iter(load):
        ...
        with torch.no_grad():
            ...
            idcs = []
            lab = 0
            labels = [0.0 for m in range(len(pred_strs))]
            for k in range(len(pred_strs)):
              wers.append(wer(targets_str, pred_strs[k]))
              idcs.append(k)
            top_1_wer = wers[0]
            n_best_hyps = [(pred_strs[k], pred_probs[k], wers[k]) for k in range(self.n_best)]
            shuffled_list = random.shuffle(n_best_hyps)
            wers_shuffled = [item[2] for item in n_best_hyps]
            n_best_hyps = [(item[0], item[1]) for item in n_best_hyps]   
            min_idx = wers.index(min(wers_shuffled))
            labels[min_idx] = 1.0            
            assert len(labels) == len(pred_strs)
            (scores, labels) = self.scorer(n_best_hyps, feats[1].unsqueeze(1), labels)
            return (scores, labels, wers_shuffled, top_1_wer, min_idx)
